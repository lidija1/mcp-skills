...
export default function ReportModal({job, currentUser, onClose, onRerun, onDelete, onNotify}) {
    ...
    const exportCsv = () => {
        ...
        const clean = rows.map(r => {
            const {_note, _lob, _provider, _variation_index, _scenario_index, ...rest} = r;
            return rest
        })
    ...
    return (
        <div ref={ref} onClick={handleBackdrop} className="report-backdrop">
            <section className="report-dialog" role="dialog" aria-modal="true" aria-label="Job report">
                ...
                <span
                    className={`report-badge ${isError || isPolicyFailed ? 'error' : job.status === 'done' ? 'success' : 'running'}`}>
                  ...
                  ID {job.id}
                    ...
                    <div className="report-actions">
                        {screenshotUrl && (
                            <button
                                onClick={withActionFeedback('screenshot', () => window.open(screenshotUrl, '_blank', 'noopener,noreferrer'))}
                                className={reportActionClass('screenshot')} type="button"
                        ...
                        {report.personas.length > 0 && (
                            <button onClick={withActionFeedback('export-csv', exportCsv)}
                                    className={reportActionClass('export-csv')} type="button"
                                    title="Export personas as CSV">
                        ...
                        {report.profileJson && (
                            <>
                                <button onClick={withActionFeedback('view-edit', openJsonEditorWindow)}
                                        className={reportActionClass('view-edit')} type="button"
                                        title="View and edit profile JSON in a new window">
                                    <Pencil size={16}/>
                                    View & Edit
                                </button>
                                <button onClick={withActionFeedback('download', downloadProfile)}
                                        className={reportActionClass('download')} type="button"
                        ...
                        {(isAssertionSuite || isUwBatch) && (
                            <button onClick={withActionFeedback('view-json', openAssertionJson)}
                                    className={reportActionClass('view-json')} type="button"
                                    title="View structured results as JSON">
                                <FileJson size={16}/>
                                View JSON
                            </button>
                        )}
                        {isAssertFlow && assertFlowData?.persona && (
                            <button onClick={withActionFeedback('view-json', () => openAssertFlowJson(assertFlowData))}
                                    className={reportActionClass('view-json')} type="button"
                                    title="View generated persona JSON">
                                <FileJson size={16}/>
                                View JSON
                            </button>
                        )}
                        ...
                        <button onClick={withActionFeedback('copy', copyReport)} className={reportActionClass('copy')}
                                type="button">
                            <Copy size={16}/>
                            Copy
                        </button>
                ...
                <>
                    ...
                    <div className="doc-section">
                        ...
                        <div className="doc-json-shell">
                            <pre
                                className="json-code-viewer">{renderJsonSyntax(report.prettyProfileJson || report.profileJson)}</pre>
                        </div>
                    </div>
                </>
            )}
        ...
        <article className="persona-list-card">
            ...
            <div className="persona-list-main">
                ...
                <div className="persona-list-grid">
                    ...
                    <DocKV label="Vehicle"
                           value={[persona.Year, persona.Make, persona.Model].filter(Boolean).join(' ')}/>
                ...
                <div className="persona-risk-row">
                    {riskFlags.length ? riskFlags.map(flag => <span key={flag}>{flag}</span>) :
                        <span className="low">Clean / Low Risk</span>}
                </div>
            </div>
        </article>
    )
...
function AssertFlowDetailsPanel({data, onClose}) {
    ...
    return (
        <div className="af-panel-overlay" onClick={e => {
...
function renderJsonSyntax(text) {
    ...
    while ((match = tokenRegex.exec(text))) {
        ...
        nodes.push(
            <span className={`json-token ${classifyJsonToken(match[0], text.slice(tokenEnd))}`}
...
const VIOLATION_TYPES = {
    FALSE_APPROVE: {
        label: 'False Approve',
        title: 'Rule engine missed a risk — risky profile was approved without referral'
    },
    FALSE_REFER: {
        label: 'False Refer',
        title: 'Rule engine over-triggered — clean profile was incorrectly flagged for review'
    },
    MISSING_CONDITION: {
        label: 'Missing Condition',
        title: 'Wrong or incomplete rule displayed on the UW referral page'
    },
    WRONG_COUNT: {label: 'Wrong Count', title: 'Fewer UW conditions appeared than expected'},
    EXTRA_CONDITIONS: {
        label: 'Extra Conditions',
        title: 'Unexpected additional rules fired that should not have triggered'
    },
          ...
          {allPass ? '✅' : '❌'} {passCount}/{total} PASS &mdash; {verdict}
            ...
            <div className="asr-table-wrap">
                <table className="asr-table">
                    ...
                    {rows.map(row => (
                        <tr key={row.num}
                            className={row.isError ? 'asr-row-error' : row.passed ? 'asr-row-pass' : 'asr-row-fail'}>
                    ...
                    {[...failRows, ...errorRows].map(row => (
                        <div key={row.num} className="asr-failure-row">
                            ...
                            <div className="asr-failure-detail">
                                ...
                                <span className="asr-kv"><em>Expected:</em> <code
                                    className="asr-val-code">{row.expected}</code></span>
                                <span className="asr-kv"><em>Actual:</em> <code
                                    className="asr-val-code fail">{row.actual}</code></span>
                            </div>
                        </div>
                    ))}
                </div>
...
function parseAssertionSuiteContent(content) {
    ...
    for (const line of lines) {
        if (!inTable && /^\|\s*#\s*\|\s*Status/i.test(line)) {
            inTable = true;
            continue
        }
        if (inTable && !pastSep && /^\|\s*-/.test(line)) {
            pastSep = true;
            continue
        }
        if (inTable && pastSep) {
...
function parseUwBatchContent(content) {
    ...
    for (const line of lines) {
        if (!inTable && /^\|\s*TC_ID\s*\|\s*Status/i.test(line)) {
            inTable = true;
            continue
        }
        if (inTable && !pastSep && /^\|\s*-/.test(line)) {
